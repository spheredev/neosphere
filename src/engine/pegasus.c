...
js_new_TextDecoder(duk_context* ctx)
{
	...
	if (!duk_is_constructor_call(ctx))
		duk_error_blame(ctx, -1, DUK_ERR_TYPE_ERROR, "constructor must be called with 'new'");
	num_args = duk_get_top(ctx);
	if (num_args >= 1)
	...
	{
		duk_error_blame(ctx, -1, DUK_ERR_TYPE_ERROR, "unsupported encoding '%s'", label);
	}
...
js_TextDecoder_decode(duk_context* ctx)
{
	...
	if (!(string = decoder_run(decoder, input, length)))
		duk_error_blame(ctx, -1, DUK_ERR_TYPE_ERROR, "data is not valid utf-8");
